...
use crate::{
    api::{BoxFuture, MaybeSend},
    rpc_message::RpcError,
};

type HandlerFn<S> =
    dyn Send + Sync + Fn(&mut S, Value) -> BoxFuture<'static, Result<Value, RpcError>>;
...
impl<S: Send + Sync + 'static> Router<S> {
    ...
    where
        ...
        R: Serialize + 'static,
        F: Fn(&mut S, P) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = Result<R, RpcError>> + MaybeSend + 'static,
    {
        ...
        let f = Box::new(
            move |state: &mut S, params: Value| -> BoxFuture<'static, Result<Value, RpcError>> {
